...
$(document).ready(function () {
  ...
  function fetchMale_teacher_count() {
    $.ajax({
      url: "controllers/ajax/fetch_male_teacher_count.php",
      type: "POST",
  ...
  function fetchFemale_teacher_count() {
    $.ajax({
      url: "controllers/ajax/fetch_female_teacher_count.php",
      type: "POST",
  ...
  function loadTeacherData() {
    $.ajax({
      url: "controllers/ajax/fetch_teacher_data.php",
      type: "POST",
  ...
  function loadTeacherCount() {
    $.ajax({
      url: "controllers/ajax/teacherTotal.php",
      type: "POST",
      success: function (data) {
  ...
  $(".search_input").on("keyup", function () {
    ...
    $.ajax({
      url: "controllers/ajax/search_teacher_data.php",
      type: "POST",
      data: {
  ...
  $("#sub_teacher_btn").on("click", function (e) {
    ...
    if (
      ...
      $("#errorMessage").html("Попълнете всички полета.").slideDown();
      setInterval(function () {
        $("#errorMessage").hide();
      }, 10000);
    } else {
      $.ajax({
        url: "controllers/ajax/insert_teacher_data.php",
        type: "POST",
        data: {
          first_name: fname,
          last_name: lname,
        ...
        success: function (data) {
          if (data == 1) {
            ...
            $("#successMessage")
              .html("Данните са добавени успешно.")
              .slideDown();
            setInterval(function () {
              $("#successMessage").hide();
            }, 10000);
  ...
  $(document).on("click", ".t_dlt_btn", function () {
    if (confirm("Наистина ли искате да изтриете този запис?")) {
      ...
      var elem = this;
      $.ajax({
        url: "controllers/ajax/del_teacher_data.php",
        type: "POST",
        data: {
          id: teacherId,
        },
        success: function (data) {
          if (data == 1) {
            ...
            $("#successMessage").html("Данните са изтрити успешно.").slideDown();
  ...
  $(document).on("click", ".t_edit_btn", function () {
    ...
    $.ajax({
      url: "controllers/ajax/update_teacher_data.php",
      type: "POST",
      data: {
        id: teacherID,
      },
  ...
  $(document).on("click", "#edit_teacher_btn", function (e) {
    ...
    $.ajax({
      url: "controllers/ajax/edit_teacher_data.php",
      type: "POST",
      data: {
        id: teacher_edit_id,
        first_name: fname,
      ...
      success: function (data) {
        if (data == 1) {
          $("#edit_modal").hide();
          loadTeacherData();
          $("#successMessage").html("Данните са актуализирани успешно.").slideDown();
          setInterval(function () {
            $("#successMessage").hide();
          }, 10000);
        } else {
  ...
  $(document).on("click", ".t_view_details", function () {
    ...
    $.ajax({
      url: "controllers/ajax/teacher_view_details.php",
      type: "POST",
      data: {
        id: t_view_id,
      },
